 ...
 * 设置：一份给人填、给界面改的配置。
 *
 * 为什么要有它：v1/v2 的模型配置要用户自己新建 `config/llm.conf` 并手写
 ...
 *   2. 旧文件 `config/llm.conf` 继续认（老用户不用动），settings.json 优先。
 */

import fs from "node:fs";
import path from "node:path";
import { readLlmConfig } from "./config.ts";
...
export type Settings = {
	...
	/** 这些值是从哪儿读来的（如实告诉用户） */
	source: "settings.json" | "llm.conf" | "default";
};
...
/** 读设置：settings.json 优先，其次兼容旧的 llm.conf，最后内置默认。 */
export function readSettings(homeDir: string): Settings {
	...
	const legacy = readLlmConfig(homeDir);
	if (legacy.provider || legacy.model || legacy.apiKey || legacy.quietStart !== undefined) {
		return {
			model: {
				mode: "provider",
			...
			quietHours: {
				start: clampHour(legacy.quietStart, DEFAULT_SETTINGS.quietHours.start),
				end: clampHour(legacy.quietEnd, DEFAULT_SETTINGS.quietHours.end),
			},
