...
/// 设置表单：模型、安静时段、主动说话。
///
/// 存在的理由很直接：以前配模型要用户自己新建 `config/llm.conf` 并手写
...
struct SettingsView: View {
	...
	var body: some View {
		ScrollView {
			VStack(alignment: .leading, spacing: compact ? 12 : 16) {
				if !compact {
					Text("设置").font(.system(size: 15, weight: .medium))
					Text("改完按「保存并应用」。模型换了内核会**当场换一次思考引擎**，不用重启应用。")
	...
	private var modelSection: some View {
		VStack(alignment: .leading, spacing: 8) {
			Text("模型").font(.system(size: 13, weight: .medium))
			Picker("用哪家", selection: $mode) {
				Text("内置供应商（只填 Key）").tag("provider")
				Text("自定义地址（OpenAI 格式等）").tag("custom")
			}
			...
			if mode == "provider" {
					...
					if !model.availableModels.isEmpty {
						Picker("模型", selection: $modelId) {
							...
							if !modelId.isEmpty, !model.availableModels.contains(where: { $0.id == modelId }) {
								Text("\(modelId)（正在用，不在清单里）").tag(modelId)
							}
						...
						TextField("模型 ID（例如 deepseek-chat）", text: $modelId)
							.textFieldStyle(.roundedBorder)
							.frame(maxWidth: 320)
					}
					Button(model.loadingModels ? "读取中…" : "读取可选模型（实时）") { model.loadModels(provider: provider) }
				...
				.frame(maxWidth: 460)
				TextField("服务地址（http:// 或 https:// 开头，例如 https://api.example.com/v1）", text: $baseUrl)
					.textFieldStyle(.roundedBorder)
				HStack(spacing: 8) {
					TextField("模型名（例如 gpt-4o-mini）", text: $modelId)
						.textFieldStyle(.roundedBorder)
						.frame(maxWidth: 220)
			...
			Text("密钥由内核保存在本机（文件权限 0600），**保存后不会再显示出来**；它只会传给思考引擎，不进日志、不进界面。")
	...
	private var shellSection: some View {
		VStack(alignment: .leading, spacing: 6) {
			Text("它的手脚").font(.system(size: 13, weight: .medium))
			Picker("档位", selection: Binding(
				get: { agentShell },
				set: { agentShell = $0 }
			)) {
				Text("全开（能跑命令、能读写文件）").tag("on")
				Text("只读（能看能搜，不能改、不能跑命令）").tag("readonly")
				Text("只剩说话（不能碰文件）").tag("off")
			}
			.labelsHidden()
			.frame(maxWidth: 420)
			Text(agentShellHint)
	...
	private var agentShellHint: String {
		switch agentShell {
		case "readonly":
			return "它能读文件、搜索，但不能写、不能跑命令。想说话/留话/改作息仍然可以（那些走内核的能力请求）。"
		case "off":
			return "它只能说话、留话、决定下次什么时候醒、向你提问——完全碰不到文件与命令。最安全，但它也就不能自己去看世界、写笔记了。"
		default:
			return "它有手有脚：能跑命令、能读写文件（写入限制在日记/探索/笔记/私人空间/工作记忆里）。**请知道：命令不是沙箱**——同一个用户下的进程能读到的东西它也能读到（比如它自己的密钥文件）。要真边界就选「只读」或「只剩说话」，或者给它单独的用户账号。"
		}
	}
	...
	private var startupSection: some View {
		VStack(alignment: .leading, spacing: 6) {
			Text("开机就在").font(.system(size: 13, weight: .medium))
			HStack(spacing: 8) {
			...
			Text("登录项在「系统设置 → 通用 → 登录项」里，你随时能自己关掉。")
				.font(.system(size: 10.5))
				.foregroundStyle(.tertiary)
		}
		.onAppear {
	...
	private var quietSection: some View {
		VStack(alignment: .leading, spacing: 6) {
			Text("安静时段（它不出声，留言照收）").font(.system(size: 13, weight: .medium))
			HStack(spacing: 6) {
				...
				if quietStart == quietEnd {
					Text("（起止相同＝关闭安静时段）").font(.system(size: 11)).foregroundStyle(.tertiary)
				}
			}
			Text("安静时段里你发的消息不会丢：它们排着队，等它醒来就看到。急着说可以点面板上的「现在叫醒它」。")
				.font(.system(size: 10.5))
				.foregroundStyle(.tertiary)
				.fixedSize(horizontal: false, vertical: true)
		}
	...
	private var proactiveSection: some View {
		VStack(alignment: .leading, spacing: 6) {
			Text("它主动找你").font(.system(size: 13, weight: .medium))
			Toggle("允许它主动说一句（不是每次都回话，也不是刷存在感）", isOn: $proactiveEnabled)
				.toggleStyle(.switch)
				.controlSize(.small)
			HStack(spacing: 6) {
				Text("每天最多").font(.system(size: 12))
				Stepper(value: $proactivePerDay, in: 0...999) {
					Text("\(proactivePerDay) 次").font(.system(size: 12, design: .monospaced))
				}
				.frame(width: 170)
			}
			.disabled(!proactiveEnabled)
			Text("想让它多说就调大（0–999，0 = 不主动说话）。这只是防刷屏的安全阀，真正的约束是安静时段——那段时间它永远不弹窗。")
				.font(.system(size: 10.5))
				.foregroundStyle(.tertiary)
				.fixedSize(horizontal: false, vertical: true)
			Text("额度是**内核**在管：超了的话它说的话会记成留言（记录里看得到），但不会弹到你眼前；安静时段永远不弹。")
				.font(.system(size: 10.5))
				.foregroundStyle(.tertiary)
				.fixedSize(horizontal: false, vertical: true)
		}
	...
	private var honestNotes: some View {
		VStack(alignment: .leading, spacing: 4) {
			Text("去哪儿找这些值").font(.system(size: 12, weight: .medium))
			Text("· DeepSeek：platform.deepseek.com → API Keys；模型填 deepseek-chat\n· 中转/自建（OpenAI 格式）：选「自定义地址」，填服务商给的 base_url 与模型名\n· Claude / OpenAI / Gemini：需要能连外网")
				.font(.system(size: 11))
				.foregroundStyle(.secondary)
				.fixedSize(horizontal: false, vertical: true)
			if let settings = model.settings {
				Text("设置文件：\(ShellModel.tilde(settings.settingsPath))（来源：\(settings.source)）")
					.help("路径按你这台电脑的实际位置显示（内核读的是当前用户的家目录），界面上缩成 ~ 以免泄露用户名")
