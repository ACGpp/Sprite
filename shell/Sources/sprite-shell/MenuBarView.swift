...
struct MenuBarView: View {
	...
	/// 提示条要不要标成橙色（"出问题了"）。
	/// 判据曾经只认中文（`contains("没能")`），英文模式下就永远不变色——所以两种语言都认。
	static func noteIsProblem(_ note: String) -> Bool {
		["没能", "失败", "Couldn't", "couldn't", "failed", "Failed"].contains { note.contains($0) }
	}
		...
		VStack(alignment: .leading, spacing: 4) {
			...
			if let setup = model.snapshot?.setup, !setup.hasModelConfig {
				HStack(spacing: 6) {
					...
					Text(L("还没配模型——它还不会思考"))
					Button(L("去填")) { model.onOpenRecords?(.settings) }
						.buttonStyle(.link)
						.font(.system(size: 11.5))
				}
				.font(.system(size: 11.5))
		...
		VStack(alignment: .leading, spacing: 4) {
			if model.isRecording {
				HStack(spacing: 6) {
					...
					Text(model.voiceStatusText.isEmpty ? L("正在录…") : model.voiceStatusText)
			...
			if let note = model.archiveNote, !model.isRecording {
				Text(note).font(.system(size: 10))
					.foregroundStyle(Self.noteIsProblem(note) ? .orange : .secondary)
					.lineLimit(2)
			}
			if model.voiceNeedsRetry, !model.isRecording, let file = model.lastVoiceFile {
				HStack(spacing: 8) {
					Button {
						...
						Label(
							model.retranscribing ? L("正在重新识别…") : (model.useSystemRecognition ? L("改用设备端再试（不出机器）") : L("改用系统识别（音频送 Apple）")),
							systemImage: "arrow.left.arrow.right"
						)
						.font(.system(size: 11))
					}
					.buttonStyle(.bordered).controlSize(.small).disabled(model.retranscribing)
					Button(L("就这样发")) { model.voiceNeedsRetry = false }
	...
	private var actionsSection: some View {
		VStack(alignment: .leading, spacing: 2) {
			actionRow(L("现在叫醒它"), symbol: "sun.max") { model.wakeNow() }
			Menu {
				Button(L("安静一小时")) { model.pause(until: "hour") }
				Button(L("今晚别出声")) { model.pause(until: "tonight") }
				Button(L("直到我说继续")) { model.pause(until: "manual") }
			} label: {
				Label(model.isPaused ? L("继续呼吸") : L("暂停呼吸"), systemImage: model.isPaused ? "play" : "moon.zzz")
			}
			...
			Toggle(isOn: Binding(
				...
				Text(model.speakAloud ? L("出声：开（在家模式）") : L("出声：关（办公模式）"))
			...
			Toggle(isOn: Binding(
				...
				Text(model.useSystemRecognition ? L("识别：系统通道（更准，音频送到 Apple）") : L("识别：设备端（不出这台机器）"))
					.font(.system(size: 12.5))
			}
			.toggleStyle(.switch)
			.controlSize(.small)
			actionRow(L("打开它的记录…"), symbol: "book") { model.onOpenRecords?(.live) }
	...
	private var footerSection: some View {
		VStack(alignment: .leading, spacing: 6) {
			...
			HStack {
				Button(showDiagnostics ? L("收起诊断") : L("诊断…")) { showDiagnostics.toggle() }
					.buttonStyle(.borderless)
					.font(.system(size: 11))
					.foregroundStyle(.secondary)
				Spacer()
