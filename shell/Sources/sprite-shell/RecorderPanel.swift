...
struct RecorderView: View {
	...
	var body: some View {
		HStack(alignment: .center, spacing: 12) {
			...
			VStack(alignment: .leading, spacing: 3) {
				HStack(spacing: 6) {
					Text(model.voicePhase == "transcribing" ? L("正在转写…") : L("正在录音"))
	...
	private var liveLine: String {
		if model.voicePhase == "transcribing" { return L("已经把听到的送去识别，稍等一下") }
		let live = model.recordingLiveText.trimmingCharacters(in: .whitespacesAndNewlines)
		if live.isEmpty { return L("在听…（说话就行，说完点「停止并转写」）") }
		return live
	}
}
