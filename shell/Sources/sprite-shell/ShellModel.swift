...
public final class ShellModel: ObservableObject {
	...
	public func saveSettings(_ patch: [String: Any], completion: (@MainActor @Sendable (Bool) -> Void)? = nil) {
		...
		requestInBackground(
			...
			guard let object = response?.objectValue else {
				settingsNote = Lf("保存失败：%@", failure ?? L("内核没有回应"))
				completion?(false)
				return
			}
			...
			if let quiet = object["quietHours"]?.objectValue {
				...
				parts.append(String(format: L("安静 %02d:00–%02d:00"), start, end))
			}
		...
		DispatchQueue.global(qos: .userInitiated).async { [self] in
			...
			onMain { [self] in
				requestInBackground(["id": UUID().uuidString, "type": "command", "command": payloadBox.value, "protocolVersion": protocolVersion], client: client) { [self] response, failure in
					guard let object = response?.objectValue else {
						archiveNote = Lf("语音没能存进记忆：%@", failure ?? L("内核没有回应"))
						completion?(false)
						return
					}
					...
					archiveNote = [file != nil ? L("语音已存进记忆") : L("只存了文字（没有音频）"), size].compactMap { $0 }.joined(separator: "　·　")
					lastVoiceFile = file
	...
	public func retranscribe(path: String, forceChannel: VoiceTranscriber.Channel? = nil) {
		...
		loadAudio(path: path) { [weak self] data in
			...
			self.retranscribeNote = Lf("正在重新识别%@…", level)
	...
	public var diagnosticsText: String {
		...
		lines.append("连接：\(connected ? "已连接" : L("未连接"))")
	...
	public init(socketPath: String) {
		...
		Loc.loadFromDefaults()
		self.uiLanguage = Loc.language
	}
	...
	public func sendMessage(_ text: String) -> Bool {
		...
		guard let client else {
			...
			connectionError = L("内核没连上，这句话没有发出去（内容已留在输入框）")
			onChange?()
			return false
		}
	...
	public func wakeNow() {
		guard let client else {
			setWakeNotice(L("内核没连上，叫不醒它"), seconds: 8)
			return
		}
		setWakeNotice(L("正在叫醒它…"), seconds: 10)
		...
		DispatchQueue.global(qos: .userInitiated).async { [self] in
			do {
				...
				onMain {
					if deferred {
						self.setWakeNotice(L("它现在不方便（安静时段）——你的话会在它下次醒来时看到"), seconds: 20)
					} else if overrode {
						self.setWakeNotice(L("已经叫醒它了（安静时段，按你的要求越过）"), seconds: 12)
					} else {
						self.setWakeNotice(L("已经叫醒它了，它正在醒来"), seconds: 12)
					}
				}
			} catch {
	...
	public func pause(until: String) {
		runPauseCommand(L("暂停")) { client in _ = try client.pause(until: until) }
	}

	public func resume() {
		runPauseCommand(L("继续")) { client in _ = try client.resume() }
	}
	...
	public var conversationSinceLaunch: [(speaker: String, text: String)] {
		...
		if let state {
			entries += state.messages.filter { $0.imported != true }.map { (L("你"), $0.text, $0.seq) }
			entries += state.outgoing.filter { $0.imported != true }.map { (L("它"), $0.text, $0.seq + 1) }
		}
		// 还没被内核确认的排在最**后**（最新的话在最下面），用大序号保证顺序稳定
		entries += optimisticIncoming.enumerated().map { (L("你（发送中）"), $0.element, Int.max - 1000 + $0.offset) }
	...
	private func apply(_ event: SpriteEvent) {
		...
		case .wakeDeferred:
			...
			let reason = event.data["reason"]?.stringValue ?? L("现在不方便")
	...
	public var presenceTitle: String {
		...
		guard connected else { return L("没有连上内核") }
		switch state?.presence {
		case .asleep: return L("它睡着了")
		case .breathing: return L("它在呼吸")
		case .thinking: return L("它在想事情")
		case .quiet: return L("它在休息")
		case .paused: return L("你让它安静一会儿")
		case .degraded: return L("它不舒服")
		case nil: return L("正在读取它的状态")
		}
	}

	/// 它今天做了什么（内核数的，外壳不猜）：面板与记录窗口显示同一份
	/// "今天做了什么"的读数。
	/// 注意：文案在**外壳**里拼（SpriteRPC 里那份 `summary` 拿不到词典，
	/// 英文模式下会露出中文——实测渲染出来才发现）。
	public var todaySummary: String? {
		guard let today = state?.today else { return nil }
		return Lf("今天：醒来 %d 次 · 写了 %d · 读了 %d · 说话 %d", today.wakes, today.writes, today.reads, today.said)
	}
	...
	/// 界面语言（跟随系统 / 中文 / English）。改动立刻生效并记住——
	/// ShellModel 是 @Published，赋值会让所有视图重新渲染，所以不需要重启应用。
	@Published public var uiLanguage: UILanguage = .auto {
		didSet {
	...
	public var quietNotice: String? {
		...
		var parts: [String] = []
		if state.presence == .quiet { parts.append(L("安静时段（23:00–07:00）它不出声")) }
		if state.presence == .paused { parts.append(L("你让它安静一会儿")) }
	...
	public var activityText: String? {
		...
		case "read": return L("正在读东西")
		case "bash": return L("正在动手做事")
		case "write", "edit": return L("正在写东西")
		case "say": return L("正在斟酌措辞")
		case .some(let tool): return "正在用 \(tool)"
		case nil: return L("正在做事")
		}
	}

	public var nextBreathText: String {
		guard connected else { return L("内核未运行") }
		guard let seconds = secondsUntilNextBreath else { return L("还没有安排下一次") }
		if seconds <= 0 { return L("该醒来了") }
	...
	public var breathLine: String {
		...
		if state?.presence == .thinking { return L("它现在醒着（正在做事）\n") + nextBreathText }
		return nextBreathText
	}

	private var currentActivityLine: String {
		switch state?.presence {
		case .thinking: return L("它现在醒着，正在做事")
		case .breathing: return L("它醒着，等下一次呼吸")
		case .quiet: return L("它在休息（安静时段）")
		case .paused: return L("你已经让它安静下来")
		case .degraded: return L("它现在不舒服（看诊断）")
		default: return L("它现在不在呼吸")
		}
	}
