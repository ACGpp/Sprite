...
struct RecordsView: View {
	...
	init(model: ShellModel, initialSection: Section = .live, autoSelectFirst: Bool = true) {
		self.model = model
	...
	var body: some View {
		NavigationSplitView(columnVisibility: $columnVisibility) {
			...
			List(entries.indices, id: \.self, selection: Binding(
				get: { selected.flatMap { sel in entries.firstIndex(where: { $0["path"] == sel["path"] && $0["at"] == sel["at"] && $0["text"] == sel["text"] }) } },
				set: { index in
					if let index { selected = entries[index]; loadDocumentIfNeeded() } else { selected = nil }
				}
			)) { index in
				let entry = entries[index]
				VStack(alignment: .leading, spacing: 3) {
					Text(entry["title"] ?? entry["who"] ?? "（无标题）")
		...
		VStack(alignment: .leading, spacing: 10) {
			HStack(spacing: 8) {
				...
				if !path.isEmpty {
					...
					.help("先用设备端识别；没听出来再走系统通道（音频会送到 Apple）")
				}
			}
			if !path.isEmpty {
				Text("音频由内核保管，播放走内核读取——外壳不碰记忆目录。")
					.font(.system(size: 10))
					.foregroundStyle(.tertiary)
			} else {
				Text("这条只有文字，没有音频")
	...
	private func reload() {
		...
		model.loadRecords(kind: section.queryKind) { list in
			...
			if autoSelectFirst, let first = list.first {
				selected = first
	...
	private func loadDocumentIfNeeded() {
		...
		guard let path = selected?["path"] else { document = ""; return }
		model.loadDocument(path: path) { text in document = text }
	}
}
