...
func spriteShellMain() {
	// 界面语言：跟随系统（默认）/ zh / en。测试与截图用 --lang en 强制英文。
	if let raw = argumentValue("--lang") {
		Loc.language = UILanguage(rawValue: raw) ?? .auto
		Loc.forced = Loc.language
	} else {
	...
	// ─── 卡死自查：用**真实窗口**跑记录窗口，看主线程还能不能按时响应 ───
	//
	// 为什么需要它：布局死循环只在真实窗口里出现（离屏渲染不会触发），
	// 而"点一下卡死"这种 bug 靠人点、靠猜都太慢。这里把窗口真的建出来、
	// 跑 3 秒 run loop，数主线程响应了多少次定时器——卡住就一次都不响。
	if let sectionArg = argumentValue("--hang-check") {
		let section: RecordsView.Section
		switch sectionArg {
		case "settings": section = .settings
		case "diary": section = .diary
		case "voice": section = .voice
		case "conversations": section = .conversations
		case "thoughts": section = .thoughts
		...
		var ticks = 0
		let timer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in ticks += 1 }
		let deadline = Date().addingTimeInterval(3)
		while Date() < deadline {
			RunLoop.current.run(mode: .default, before: Date().addingTimeInterval(0.02))
		}
		timer.invalidate()
		let cpu = Double(clock()) / Double(CLOCKS_PER_SEC)
		print("section=\(sectionArg) 主线程响应 \(ticks) 次（3 秒；卡住会是 0）CPU 累计 \(String(format: "%.2f", cpu))s")
		window.close()
		if ticks >= 15 {
			print("结论: PASS ✓ 记录窗口-\(sectionArg) 不卡")
			exit(0)
		}
		print("结论: FAIL ✗ 记录窗口-\(sectionArg) 卡住主线程（布局死循环）")
		exit(1)
	}
	...
	if let dir = argumentValue("--shot") {
		...
		let selectArg = Int(argumentValue("--select") ?? "0") ?? 0
		let section: RecordsView.Section = {
			switch sectionArg {
		...
		shoot(RecordsView(model: model, initialSection: .live, initialSelection: selectArg), size: NSSize(width: 900, height: 580), name: "记录窗口-现在", opaque: true)
		shoot(MenuBarView(model: model, initialDiagnostics: true), size: NSSize(width: 340, height: 464), name: "面板-诊断展开", opaque: true)
		shoot(
			RecordsView(
				model: model,
				initialSection: section,
				autoSelectFirst: !CommandLine.arguments.contains("--no-autoselect"),
				initialSelection: selectArg
			),
	...
	if selfTest {
		...
		let editMenu = NSApp.mainMenu?.items.compactMap(\.submenu).first { $0.title == L("编辑") }
		...
		// 语言开关：英文模式下查词典，没翻译的回落中文（不许出现空字符串或 key）
		Loc.language = .en
		check("英文模式下会翻译", L("现在叫醒它") == "Wake it now", L("现在叫醒它"))
		check("没翻译的文案回落中文（不留空白）", L("这条肯定没翻译过") == "这条肯定没翻译过")
		check("带参数的文案保留占位符", Lf("第 %d 次呼吸", 7) == "Breath #7", Lf("第 %d 次呼吸", 7))
		check("中文模式下原样返回", { Loc.language = .zh; return L("现在叫醒它") == "现在叫醒它" }())
		Loc.language = .auto
		...
		model.wakeNow()
		check("未连接时叫醒会说清楚", (model.wakeNotice ?? "") != (noticeBefore ?? "") && (model.wakeNotice ?? "").contains(L("内核没连上，叫不醒它")))
		let droppedSend = model.sendMessage("这句话不该被悄悄丢掉")
		check("未连接时发送返回失败", droppedSend == false)
		check("未连接时发送给出原因", (model.connectionError ?? "").contains(L("内核没连上，这句话没有发出去（内容已留在输入框）")))
