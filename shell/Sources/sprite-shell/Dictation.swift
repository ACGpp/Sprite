...
public final class DictationService: NSObject, @unchecked Sendable {
	...
	public func start() {
		...
		if AVCaptureDevice.authorizationStatus(for: .audio) == .denied || SFSpeechRecognizer.authorizationStatus() == .denied {
			state = .needsPermission("权限被拒。去「系统设置 → 隐私与安全性」里允许「麦克风」与「语音识别」")
			return
		}
		// 只在"未询问"时申请一次
		requestOnce { [weak self] granted in
			...
			if granted {
				...
				self.state = .needsPermission("没有授权，无法录音。去「系统设置 → 隐私与安全性」里允许")
			}
		}
	}
			...
			onMain { [self] in
				...
				if hadError {
					...
					if self.state.isRecording {
						...
						self.state = .failed("识别中断，已保留听到的内容")
					}
				}
			}
		}
	...
	private func createAudioFile(matching format: AVAudioFormat) {
		...
		lastCaptureFailed = "录音文件建不起来，只保留转写文字"
	}
	...
	private func writeAudio(_ buffer: AVAudioPCMBuffer) {
		...
		do {
			if audioFile.processingFormat == buffer.format {
				...
				lastCaptureFailed = "录音格式变了，这一段没写进原话"
				self.audioFile = nil
				return
			}
	...
	private func finalizeAudio() -> (URL?, Int) {
		...
		if let level = VoiceTranscriber.level(url: url), !level.hasVoice {
			lastCaptureFailed = "⚠︎ 这段的原话没能录上（音频里几乎没有声音），只留了文字"
		}
	...
	private func finish() {
		...
		guard audio != nil || !streamed.isEmpty else {
			finishing = false
			state = .failed("没有听到内容（可以说长一点，或检查麦克风输入）")
			return
		}
	...
	private func emit(text: String, audio: URL?, durationMs: Int) {
		...
		guard !text.isEmpty else {
			...
			if let level = lastLevel, !level.hasVoice {
				state = .failed(String(format: "麦克风几乎没收到声音（%.1f 秒，峰值 %.2f）——检查一下输入设备；录音已保留", level.seconds, level.peak))
			} else {
				state = .failed("听到了声音但没识别出文字，录音已保留（可以换识别通道再试）")
			}
			return
		}
		state = .done(text)
