...
public final class SetupPanel: NSPanel {
	...
	private func build() {
		...
		let kicker = NSTextField(labelWithString: L("SPRITE · 第一次见面"))
		...
		let heading = NSTextField(labelWithString: L("让它在你的 Mac 上醒来"))
		...
		subtitleLabel.stringValue = L("记忆只留在本机（~/.claude-memory）。完成下面三步，之后打开应用就能直接继续。")
		...
		// 以前这一步是"去配置文件里手写模型名与密钥"——对不懂命令行的人是道墙。
		...
		let recheck = NSButton(title: L("重新检测"), target: self, action: #selector(handleRecheck))
		recheck.frame = NSRect(x: 24, y: 20, width: 100, height: 30)
		recheck.bezelStyle = .rounded
		container.addSubview(recheck)

		let openConfig = NSButton(title: L("打开配置目录"), target: self, action: #selector(handleOpenConfig))
		openConfig.frame = NSRect(x: 130, y: 20, width: 120, height: 30)
		openConfig.bezelStyle = .rounded
		container.addSubview(openConfig)

		let speak = NSButton(title: L("说第一句话"), target: self, action: #selector(handleSpeak))
	...
	public func update(setup: SetupReport?, hasSpoken: Bool) {
		...
		guard let setup else {
			subtitleLabel.stringValue = L("还没有连上内核，读不到它的状态。内核是唯一的真相来源——先让它跑起来。")
			return
		}
		let steps = setup.steps(hasSpoken: hasSpoken)
		if steps.allSatisfy(\.done) {
			subtitleLabel.stringValue = L("三步都完成了。它会自己醒来、自己待着，偶尔跟你说句话。")
		} else {
			subtitleLabel.stringValue = Lf("记忆只留在本机（%@）。下面三步：填模型 → 说第一句话。配置不用你新建文件，在表里填就行。", ShellModel.tilde(setup.homeDir))
		}
