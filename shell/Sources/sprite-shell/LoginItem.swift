...
public enum LoginItem {
	public enum Status: Equatable {
		...
		public var describe: String {
			switch self {
			case .enabled: return "登录时会自动打开"
			case .disabled: return "登录时不会自动打开"
			case .requiresApproval: return "已登记，等你在「系统设置 → 通用 → 登录项」里允许"
	...
	public static func status() -> Status {
		guard #available(macOS 13.0, *) else { return .unavailable("系统版本太旧（需要 macOS 13+）") }
		let service = SMAppService.mainApp
		switch service.status {
		case .enabled: return .enabled
		case .notRegistered: return .disabled
		case .requiresApproval: return .requiresApproval
		case .notFound: return .unavailable("系统没找到这个应用（先把它放进「应用程序」再试）")
	...
	public static func setEnabled(_ enabled: Bool) -> (Status, String?) {
		guard #available(macOS 13.0, *) else { return (.unavailable("系统版本太旧（需要 macOS 13+）"), nil) }
		let service = SMAppService.mainApp
		do {
			if enabled {
