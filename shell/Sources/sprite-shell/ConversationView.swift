...
struct ConversationView: View {
	...
	private var header: some View {
		HStack(spacing: 6) {
			if !compact {
				Text(L("现在")).font(.system(size: 13, weight: .medium))
				if let name = model.companionName {
					Text(Lf("和 %@ 说话", name)).font(.system(size: 11)).foregroundStyle(.secondary)
				}
			} else {
				Text(L("对话")).font(.system(size: 10, weight: .medium)).foregroundStyle(.tertiary)
			}
			Spacer()
			if let onExpand, compact {
				Button(L("展开")) { onExpand() }
		...
		//
		// 尺寸必须**稳定**：lineLimit(1) + 撑满宽度，而不是用 Spacer 参与分配——
		// 否则窗口重排时这两行跟着变宽变窄，就成了布局循环的燃料（实证见 AppDelegate 里
		// `controller.sizingOptions = []` 那段注释：这条链子曾让记录窗口卡死）。
		if !compact {
			HStack(spacing: 8) {
				...
				if let hint = model.idleHint {
					Text(hint).font(.system(size: 11)).foregroundStyle(.orange).lineLimit(1)
				}
			}
				...
				VStack(alignment: .leading, spacing: compact ? 6 : 10) {
					if visibleTurns.isEmpty {
						VStack(alignment: .leading, spacing: 3) {
							Text(model.connected ? L("还没有新对话。说一句试试。") : L("还没连上内核（它是唯一的事实来源）"))
								.font(.system(size: compact ? 11 : 12.5))
								.foregroundStyle(.tertiary)
							if model.archivedTurnCount > 0 {
								Text(Lf("更早的%d条旧对话在「对话记录」里", model.archivedTurnCount))
	...
	private func bubble(_ turn: ShellModel.Turn) -> some View {
		VStack(alignment: turn.fromMe ? .trailing : .leading, spacing: 2) {
			HStack(spacing: 5) {
				Text(turn.fromMe ? L("我") : (model.companionName ?? L("它")))
					.font(.system(size: 10))
					.foregroundStyle(.tertiary)
				...
				if turn.imported {
					Text(L("旧记录")).font(.system(size: 9.5)).foregroundStyle(.tertiary)
				}
				if turn.heldBack {
					// 安静时段收着的：让它可见，并说明它当时为什么没弹出来
					Text(L("安静时段收着的")).font(.system(size: 9.5)).foregroundStyle(.tertiary)
				}
			}
			Text(turn.text)
	...
	private var thinkingRow: some View {
		HStack(spacing: 6) {
			...
			Text(L("它在想…已经 \(model.thinkingSeconds) 秒"))
	...
	private var inputRow: some View {
		HStack(spacing: 8) {
			...
			TextField(L("跟它说一句…"), text: $model.draft)
			...
			.buttonStyle(.borderless)
			.help(model.isRecording ? L("停止并转写") : L("点一下开始录音，再点一下停止并转写"))
			Button(L("送出"), action: send)
