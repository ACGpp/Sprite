...
public final class AppDelegate: NSObject, NSApplicationDelegate {
	...
	private func wireDictation() {
		dictation.onTranscript = { [weak self] text in
			...
			self.model.voiceStatusText = "转写完成，已放进输入框"
		}
		...
		dictation.onVoiceNote = { [weak self] text, url, durationMs in
			...
			if text.isEmpty {
				...
				self.model.voiceStatusText = String(format: "录音结束（%.1f 秒）：设备端没听出内容", seconds)
			} else {
				self.model.appendTranscript(text)
				self.model.voiceStatusText = String(format: "录音结束（%.1f 秒），正在存进记忆…", seconds)
			}
			self.model.commitVoice(transcript: text, audioURL: url, durationMs: durationMs) { [weak self] ok in
				...
				self.model.voiceStatusText = text.isEmpty ? "\(note)　—— 没听出文字，可以点下面的「重新识别」" : note
			}
		}
		dictation.onLiveText = { [weak self] live in
			guard let self else { return }
			self.model.recordingLiveText = String(live.suffix(120))
			self.model.voiceStatusText = "正在录…听到：" + String(live.suffix(30))
		}
		dictation.onStateChange = { [weak self] state in
			...
			case .recording:
				self.model.voiceStatusText = "正在录…再点一下停止"
				// 录制期间面板可能被关掉：图标上要留一个正在录的记号，
				// 否则用户不知道麦克风还开着（"点一下面板就没了"的现场）
				self.updateStatusIcon()
			case .transcribing: self.model.voiceStatusText = "正在转写…"
			case .done(let text): self.model.voiceStatusText = "转写完成：" + String(text.prefix(30))
	...
	private func noteTranscriptQuality(text: String, speakingSeconds: Double? = nil) {
		...
		let channel = model.useSystemRecognition ? "系统识别" : "设备端识别"
		...
		if ShellModel.looksIncomplete(text: text, seconds: seconds, speakingSeconds: speakingSeconds) {
			...
			let speechNote = speakingSeconds.map { String(format: "其中约 %.0f 秒有说话声", $0) } ?? String(format: "共 %.0f 秒", seconds)
			model.voiceSummary = "⚠︎ " + speechNote + String(format: "，只转出 %d 字（%@）——发之前先看一眼，或换个通道再试", characters, channel)
			model.voiceNeedsRetry = model.lastVoiceFile != nil
		} else {
			let speechNote = speakingSeconds.map { String(format: "（约 %.0f 秒说话声）", $0) } ?? ""
			model.voiceSummary = String(format: "录音 %.0f 秒%@，转写 %d 字（%@）", seconds, speechNote, characters, channel)
	...
	private func openRecords(_ request: RecordsSectionRequest = .live) {
		if recordsWindow == nil {
			let controller = NSHostingController(
				rootView: RecordsView(model: model, initialSection: {
					switch request {
