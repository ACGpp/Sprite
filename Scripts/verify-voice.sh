...
"$SHELL_BIN" --transcribe voice/不存在.m4a --socket "$SOCKET" >/dev/null 2>&1 || true
# 录 2 秒，量电平：几乎没有声音就跳过（这属于"环境不具备条件"，不是产品缺陷）
"$SHELL_BIN" --record-test 2 --socket "$SOCKET" > "$HOME_DIR/probe.log" 2>&1 || true
...
SAY_PID=$!
"$SHELL_BIN" --record-test "$SECONDS_TO_RECORD" --socket "$SOCKET"
RC=$?
...
"$SHELL_BIN" --shot "$SHOT_DIR" --section voice --socket "$SOCKET" 2>&1 | grep -E "✓|✗|连接|结论" | sed 's/^/  /'
"$SHELL_BIN" --shot "$SHOT_DIR" --section voice --socket "$SOCKET" 2>&1 | sed -n '/记录窗口-voice.png 屏上文字/,$p' | sed 's/^/  /'
